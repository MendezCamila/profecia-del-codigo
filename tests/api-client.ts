...
export async function obtenerDesafioAPI(tituloManuscrito: string, codigoDesbloqueo: string, siglo: string): Promise<any> {
  try {
    ...
    try {
      ...
      console.log(`📡 Error en la llamada a la API: ${error.message}`);
    ...
    if (error.code === 'ECONNREFUSED' || error.code === 'ENOTFOUND') {
      ...
      throw new Error(`Error de conexión: No se pudo conectar con la API - ${error.message}`);
      ...
      throw new Error(`Error de API: ${error.response.status} - ${JSON.stringify(error.response.data || {})}`);
      ...
      console.log(`❌ Error al configurar la solicitud a la API: ${error.message}`);
